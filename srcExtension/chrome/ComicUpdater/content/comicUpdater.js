    ...
    return {
        run : function () {
            ...
            while (folderList.length > 0)
            {
                ...
                for (var i = 0; i < rootNode.childCount; i ++)
                {
                    ...
                    else
                    {
                        ...
                        for (var j=1; j<node.uri.length && j<content.document.URL.length; j++)
                        {
                            if (node.uri.substr(0,j)==content.document.URL.substr(0,j))
                                numMatchingChars = j;
                            else
                                break;
                        }
            ...
            else if (closestBookmarkList.length == 1)
            {
                ...
                bookmarksService.changeBookmarkURI(closestBookmarkList[0].itemId, newURI);
            }
            else
            {
                var nodeList = "";
                ...
                alert("Could not find a unique closest bookmark. Found " + closestBookmarkList.length + " closest bookmarks:\n" + nodeList);
            }
        }
    };
}();